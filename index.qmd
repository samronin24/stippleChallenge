...
## Blue Noise Stippling Overview

Blue noise stippling recreates imagery with evenly dispersed dots whose density reflects underlying luminance. The method follows Bart Wronski’s progressive importance sampling framework and adds smooth downweighting of extreme shadows and highlights to maintain mid-tone fidelity.

## Loading the Source Image

The analysis uses `sam_headshot.jpg`, converted to grayscale for consistent luminance scaling.
...
```
Ensure the source file `sam_headshot.jpg` is stored in the project root or update `img_path` accordingly.
...
The next figure compares the grayscale input, computed importance map, and resulting stipple distribution.
...
#| message: false
#| warning: false
#| fig-cap: "Source image, importance map, and blue-noise stippling output"
#| fig-width: 7
#| fig-height: 4
...
The visualization confirms that the importance map concentrates weight in mid-tone facial features, producing a stipple field that preserves contours while maintaining even spacing.

## Progressive Stippling Animation

Now let’s build a GIF that shows the stippling pattern as points accumulate.
...
![Progressive stippling animation with cumulative point milestones.](progressive_stippling.gif)

The animation verifies smooth density growth: early frames prioritize mid-tone features, while later iterations fill background regions without producing visible clusters.
